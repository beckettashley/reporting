...
import { Search, ChevronDown, ChevronUp, ChevronRight, SlidersHorizontal, X, Play } from "lucide-react"
          ...
          <div className="py-1 max-h-48 overflow-y-auto">
            {filtered.length > 0 && (
              <label className="flex items-center gap-2.5 px-3 py-2 cursor-pointer hover:bg-muted/50 transition-colors border-b border-border/50">
                <input type="checkbox" checked={filtered.every((o) => selected.includes(o))}
                  onChange={() => {
                    if (filtered.every((o) => selected.includes(o))) { onChange(selected.filter((s) => !filtered.includes(s))) }
...
// ─── Inline observation (no separate card — expands as continuation of row)

function InsightObservation({ ins }: { ins: InsightRow }) {
  return (
...
function AdPreviewModal({ ins, onClose }: { ins: InsightRow; onClose: () => void }) {
  return (
    <>
      <div className="fixed inset-0 bg-black/40 z-[9998]" onClick={onClose} />
      <div className="fixed left-1/2 -translate-x-1/2 top-[10vh] z-[9999] w-[520px] max-w-[95vw] bg-background border rounded-2xl shadow-2xl flex flex-col overflow-hidden">
        <div className="flex items-center justify-between px-5 py-3.5 border-b">
          <div>
            <p className="text-sm font-semibold">{ins.adName}</p>
            <p className="text-xs text-muted-foreground">{ins.adType === "video" ? "Video ad" : "Image ad"}</p>
          </div>
        ...
        <div className="aspect-square bg-muted flex items-center justify-center">
          {ins.adType === "video" ? (
            <div className="text-center">
              <div className="w-16 h-16 rounded-full bg-foreground/10 flex items-center justify-center mx-auto mb-2">
                <Play className="h-8 w-8 text-foreground/50 ml-1" />
              </div>
...
type InsightSortKey = "adName" | "product" | "insight" | "date"
type SortDir = "asc" | "desc"

function SortHeader({ label, sortKey: sk, currentSort, currentDir, onSort, align = "left" }: {
  label: string; sortKey: InsightSortKey; currentSort: InsightSortKey; currentDir: SortDir; onSort: (k: InsightSortKey) => void; align?: "left" | "right"
}) {
...
function sortInsights(a: InsightRow, b: InsightRow, key: InsightSortKey, dir: SortDir): number {
  let cmp = 0
  switch (key) {
    case "adName": cmp = a.adName.localeCompare(b.adName); break
    case "product": cmp = a.product.localeCompare(b.product); break
    case "insight": cmp = a.insight.localeCompare(b.insight); break
...
export default function InsightsPage() {
  ...
  const [sortKey, setSortKey] = useState<InsightSortKey>("date")
  const [sortDir, setSortDir] = useState<SortDir>("desc")
  const [expandedId, setExpandedId] = useState<string | null>(null)
  const [previewInsight, setPreviewInsight] = useState<InsightRow | null>(null)
  const [showFilters, setShowFilters] = useState(false)
  const filterRef = useRef<HTMLDivElement>(null)

  const handleSort = (key: InsightSortKey) => {
  ...
  const filtered = useMemo(() => {
    ...
    result = [...result].sort((a, b) => sortInsights(a, b, sortKey, sortDir))
    return result
  }, [search, campaignFilters, adSetFilters, adFilters, productFilters, destinationFilters, sortKey, sortDir])
